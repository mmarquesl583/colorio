...
export default function WaitingScreen({ conn }: { conn: RoomConnection }) {
  ...
  return (
    <div className="corio-wide" style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', padding: '8px 16px 12px', gap: 8 }}>
      ...
      <div className="corio-card" style={{ flex: 'none', background: '#12121a', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 13, padding: '8px 10px' }}>
        <div className="corio-eyebrow" style={{ fontSize: 8, fontWeight: 700, letterSpacing: 0.6, color: 'rgba(244,242,248,0.6)', marginBottom: 6 }}>⚙️ CONFIGURAÇÕES DA SALA</div>
          ...
          {slots.map((p, i) => p ? (
            <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 8px', borderRadius: 10, background: 'rgba(255,255,255,0.03)', flex: 'none' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div className="corio-card-sub" style={{ fontSize: 8.5, fontWeight: 600, color: '#4ADE80' }}>Pronto para jogar!</div>
              </div>
